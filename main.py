        ...
        try:
            ...
            self.conn.execute(
                """INSERT OR IGNORE INTO evidence
                   ...
                   VALUES (?, 'Interview Problem', ?, ?)""",
                (
                    self.sql_concepts.text() or self.sql_topic.text() or "SQL",
                    title,
                    "Validated interview-practice submission saved at " + relative_path
                    + (". Notes: " + self.sql_notes.toPlainText().strip() if self.sql_notes.toPlainText().strip() else ""),
                ),
            )
            self.conn.commit()
