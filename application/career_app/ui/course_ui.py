...
from PySide6.QtGui import (
    QColor, QFont, QFontDatabase, QPainter, QPainterPath, QPen, QPolygonF, QSyntaxHighlighter, QTextCharFormat,
...
class AcademySpeechBubble(QFrame):
    """Right-tailed lesson callout used for the approved Academy layout."""

    def __init__(self, body: str, parent: QWidget | None = None):
        super().__init__(parent)
        self.setObjectName("AcademyWhyBubble")
        self.setMinimumWidth(0)
        self.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground, True)
        layout = QHBoxLayout(self)
        layout.setContentsMargins(16, 12, 22, 22)
        ...
        icon.setStyleSheet(
            "background:#246BFF;color:#FFFFFF;border-radius:21px;"
        ...
        title = QLabel("Why does this matter?")
        title.setStyleSheet("color:#68A1FF;font-size:12pt;font-weight:750;")
        text_layout.addWidget(title)
        body_label = QLabel(rich_text(body))
        body_label.setTextFormat(Qt.TextFormat.RichText)
        body_label.setWordWrap(True)
        body_label.setMinimumWidth(0)
        body_label.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Preferred)
        body_label.setStyleSheet("color:#D8E2F1;font-size:10pt;")
    ...
    def paintEvent(self, event) -> None:  # noqa: N802 - Qt API
        ...
        left = 0.75
        top = 0.75
        right = max(left + 1.0, float(self.width()) - 0.75)
        bottom = max(top + 1.0, float(self.height()) - tail_height - 0.75)
        radius = 12.0
        tail_left = max(left + radius, right - 46.0)
        tail_base = max(tail_left + 8.0, right - 20.0)
        tail_tip_y = bottom + 12.0

        # Build one continuous outline. Adding a rounded rectangle and a second
        # polygon produced overlapping border segments, which appeared as a
        # broken tail in the live Qt renderer.
        path = QPainterPath()
        ...
        path.lineTo(tail_base, bottom)
        path.lineTo(tail_base, tail_tip_y)
        path.lineTo(tail_left, bottom)
        ...
        painter.fillPath(path, QColor("#0E203A"))
        painter.setPen(QPen(QColor("#24569C"), 1.25))
        painter.drawPath(path)
        painter.end()


class AcademySection(QWidget):
    """Borderless Academy section with an icon and full-width accent rule."""
    ...
    def __init__(
        ...
        outer.setContentsMargins(2, 3, 0, 3)
        outer.setSpacing(12)
        ...
        icon_label.setStyleSheet(
            f"color:{accent};font-size:19pt;background:transparent;border:none;"
        )
        outer.addWidget(icon_label, 0, Qt.AlignmentFlag.AlignTop)
        ...
        title_label.setStyleSheet(f"color:{accent};font-size:11.5pt;font-weight:750;")
        ...
        header.addWidget(line, 1, Qt.AlignmentFlag.AlignVCenter)
        content.addLayout(header)
        self.body_layout = QVBoxLayout()
        self.body_layout.setContentsMargins(0, 0, 0, 0)
        self.body_layout.setSpacing(9)
        content.addLayout(self.body_layout)
...
class CoursePageWidget(QWidget):
    ...
    def _add_academy_title(
        ...
        pill.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Preferred)
        pill.setStyleSheet(
            "background:#6B35D5;color:#FFFFFF;border-radius:10px;padding:4px 11px;"
            "font-size:8.5pt;font-weight:750;"
        )
        column.addWidget(pill, 0, Qt.AlignmentFlag.AlignLeft)
        title_label = QLabel(title)
        self._title_label = title_label
        title_label.setWordWrap(True)
        title_label.setMinimumWidth(0)
        title_label.setSizePolicy(QSizePolicy.Policy.Ignored, QSizePolicy.Policy.Preferred)
        title_label.setStyleSheet("color:#FFFFFF;font-size:23pt;font-weight:750;")
        ...
        if subtitle:
            ...
            subtitle_label.setMinimumWidth(0)
            subtitle_label.setStyleSheet(
                "color:#C0CADB;background:transparent;border:none;font-size:9.5pt;"
            )
            column.addWidget(subtitle_label)
        self.page_layout.addLayout(column)

    @staticmethod
    def _clean_academy_section(markdown: str) -> str:
        ...
        while lines and not lines[0].strip():
            lines.pop(0)
        # The section already has its own colored heading and full-width rule.
        # Strip a leading Markdown heading from the body so lessons do not show
        # a second black-bar title such as “Build formulas that keep working.”
    ...
    def _add_academy_section(
        ...
        *,
        ...
        body = self._clean_academy_section(markdown)
        if not body:
            return
        section = AcademySection(title, icon, accent)
        self._render_blocks(self._parser.parse(body), section.body_layout)
        self.page_layout.addWidget(section)

    def set_academy_lesson(
        self,
        *,
        title: str,
        eyebrow: str,
        ...
        """Render the approved Academy lesson composition.

        The Academy uses a right-tailed purpose callout, then borderless concept,
        example, and task sections with full-width accent rules. Page actions are
        deliberately omitted because bookmarks and overflow controls are not part
        of the Academy workflow.
        """
        self.header_actions_host.hide()
        self.embedded_host.hide()
        self.header_actions_host.setParent(self)
        ...
        clear_layout(self.page_layout)
        self._add_academy_title(title, eyebrow, subtitle)
        purpose = str(why_it_matters or "").strip()
        if purpose:
            self.page_layout.addWidget(AcademySpeechBubble(purpose))
        self._add_academy_section(
            title=concept_title or "Learn the idea",
            icon="📖",
            accent="#21C77A",
            markdown=concept_body,
        )
        self._add_academy_section(
            title="Example",
            icon="💡",
            accent="#FFC928",
            markdown=example_body,
        )
        self._add_academy_section(
            title="Task",
            icon="🎯",
