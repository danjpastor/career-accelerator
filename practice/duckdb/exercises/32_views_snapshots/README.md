# DuckDB Exercise 32: Create reusable views and analytical snapshots
...
1. Create a reusable view that joins delivered orders to customer region, then return the view rows.
2. Query the reusable view to summarize delivered revenue by region.
3. Create a physical snapshot table from the view and return its row count.
4. Compare the view and snapshot row counts to verify the snapshot was created correctly.
