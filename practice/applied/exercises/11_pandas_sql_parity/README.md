...
SQL-to-pandas translation, joins, groupby, parity testing

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to translate a completed SQL analysis into pandas and prove both results match..
...
3. List the concepts you expect to use: SQL-to-pandas translation, joins, groupby, parity testing.
...
1. Choose a completed DuckDB analysis. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Implement the same logic in pandas. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Normalize sort order, types, and rounding. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Compare keys, rows, and numeric differences programmatically. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Explain syntax and workflow differences. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: More than row count is compared.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Numeric tolerance is explicit.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Any intentional difference is documented.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: pandas implementation.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Matching SQL reference.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Automated parity check.. Make its purpose, scope, and source clear to a reviewer.
