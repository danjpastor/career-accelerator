...
Recreate one supplied SQL summary in pandas and compare the two outputs.
...
1. Read the assignment and restate the goal in one sentence: Recreate one supplied SQL summary in pandas and compare the two outputs.
...
1. Read the SQL result definition and identify its filters, join, and grouping grain.
2. Build the equivalent pandas operations in small steps.
3. Compare row counts, keys, and one numeric total between the SQL and pandas outputs.

### Required output

One pandas implementation. One short parity comparison.

### Check your work

- Both results use the same scope and grain.
- Keys and row counts match.
- Any numeric difference is explained rather than hidden.
...
1. Verify: Both results use the same scope and grain.
2. Verify: Keys and row counts match.
3. Verify: Any numeric difference is explained rather than hidden.
