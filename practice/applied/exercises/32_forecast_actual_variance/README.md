...
budget, forecast, actual, absolute variance, percentage variance, favorable and unfavorable, contribution analysis

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to reconcile planned and actual results, isolate the largest drivers, and communicate whether each variance is favorable or unfavorable..
...
3. List the concepts you expect to use: budget, forecast, actual, absolute variance, percentage variance, favorable and unfavorable, contribution analysis.
...
1. Define sign conventions for revenue and cost variances. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Calculate absolute and percentage variance. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Rank departments or products by contribution to total variance. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Separate volume, rate, and timing explanations where possible. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Write an executive variance commentary with an action owner. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Sign conventions are explicit.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Driver contributions reconcile to total variance.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Percentage variance handles zero or very small forecasts safely.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Variance table.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Top-driver analysis.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Executive commentary.. Make its purpose, scope, and source clear to a reviewer.
