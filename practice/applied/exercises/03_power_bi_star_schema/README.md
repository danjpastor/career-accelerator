...
Model the cleaned data at a clear grain using facts and dimensions.

## Skills you will apply

fact and dimension tables, relationships, date table, filter direction

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to model the cleaned data at a clear grain using facts and dimensions..
...
3. List the concepts you expect to use: fact and dimension tables, relationships, date table, filter direction.
...
1. Define the grain of shot and time-entry facts. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Create project, artist, department, status, and date dimensions. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Build one-to-many single-direction relationships. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Mark the date table and hide technical fields. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: No accidental many-to-many relationships.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Dimension keys are unique.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: A project filter affects both facts correctly.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Model-view screenshot.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Relationship inventory.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Grain statement for each fact.. Make its purpose, scope, and source clear to a reviewer.
