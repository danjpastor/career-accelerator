...
Power Query import, data types, column profiling, source documentation

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to load operational exports, inspect their grain and quality, and document what must be corrected before modeling..
...
3. List the concepts you expect to use: Power Query import, data types, column profiling, source documentation.
...
1. Import the supplied projects, shots, and time-entry files. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Assign data types and use Power Query profiling views. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Document missing values, duplicates, suspicious ranges, and source grain. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Rename queries and fields for stakeholder readability. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Use comments, readable names, or labeled worksheet sections so another learner can follow the order of operations.
6. After each major step, compare row counts and unique entity counts with the prior stage before moving on.
7. Keep exact solutions out of notes copied from external sources; explain why the method fits this business question in your own words.
...
1. Verify this requirement independently: Every table has a stated grain and candidate key.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Dates and numeric fields use correct data types.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: At least three quality observations are supported by evidence.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Power BI file or screenshots of the imported queries.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Profiling table with issue, impact, and proposed fix.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Source and refresh assumptions note.. Make its purpose, scope, and source clear to a reviewer.
