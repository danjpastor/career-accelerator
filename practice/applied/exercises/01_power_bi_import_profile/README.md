# Applied Lab 14: Import and profile one table in Power BI
...
Import one operational table into Power BI, correct its data types, and record three useful profile observations.
...
1. Read the assignment and restate the goal in one sentence: Import one operational table into Power BI, correct its data types, and record three useful profile observations.
...
1. Import the supplied table and confirm its row count and column names.
2. Correct the date, numeric, and category data types that need attention.
3. Record three observations from column quality, distribution, or profile views.

### Required output

One Power BI file with the profiled query. Three short profile observations.
...
1. Verify: The imported row count matches the source.
2. Verify: Key dates and measures use appropriate data types.
3. Verify: The observations come from visible profile evidence.
