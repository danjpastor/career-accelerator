...
reconciliation, scope differences, exclusions, grain, audit trail

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to explain why a dashboard differs from a finance report and produce an auditable bridge..
...
3. List the concepts you expect to use: reconciliation, scope differences, exclusions, grain, audit trail.
...
1. Compare scope, dates, grain, filters, and currency. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Build a bridge between totals. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Identify each difference group. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Determine what requires correction. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Write a neutral stakeholder update. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Bridge sums exactly.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Difference groups are mutually exclusive.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Confirmed and open causes are separated.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Reconciliation bridge.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Corrected logic or explanation.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Stakeholder update.. Make its purpose, scope, and source clear to a reviewer.
