...
Translate analysis into a decision-oriented message for a nontechnical manager.

## Skills you will apply

problem, finding, impact, recommendation, limitation, audience adaptation

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to translate analysis into a decision-oriented message for a nontechnical manager..
...
3. List the concepts you expect to use: problem, finding, impact, recommendation, limitation, audience adaptation.
...
1. Write five sentences: problem, finding, impact, recommendation, limitation. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Expand them into a one-page memo. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Use one visual only if it improves the decision. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Remove unnecessary technical details. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. End with a requested action. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Recommendation follows from evidence.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Limitation changes interpretation strength.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: The requested decision is explicit.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Five-sentence summary.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: One-page memo.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Note on omitted technical detail.. Make its purpose, scope, and source clear to a reviewer.
