...
Turn the assignment into a clear analytical question before using a tool. The lab objective is to review an existing Power BI project for transformation, model, refresh, and report-performance opportunities..
...
3. List the concepts you expect to use: query folding, model size, cardinality, storage mode, measure versus column, incremental refresh, Performance Analyzer.
...
1. Identify transformations that can preserve query folding. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Remove unused columns and reduce high-cardinality text where appropriate. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Review calculated columns versus measures. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Document Import, DirectQuery, or composite-model tradeoffs. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Draft an incremental-refresh policy. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
6. Use or simulate Performance Analyzer findings and prioritize fixes. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Optimizations preserve analytical meaning.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Performance claims are supported by a measurement or documented expectation.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: The incremental-refresh policy includes date boundaries and retention.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Performance review checklist.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Before-and-after model or refresh observations.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Prioritized optimization plan.. Make its purpose, scope, and source clear to a reviewer.
