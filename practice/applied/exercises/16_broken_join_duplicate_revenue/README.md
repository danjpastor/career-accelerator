...
Turn the assignment into a clear analytical question before using a tool. The lab objective is to find why a plausible query overstates revenue and correct it without hiding duplicates..
...
3. List the concepts you expect to use: join cardinality, grain, duplicate amplification, reconciliation.
...
1. Run or inspect the broken analysis first and save its output as the before state.
2. Separate the business-definition problem from the technical implementation problem.
3. Identify the row grain before and after every join, grouping, filter, or calculation.
4. Plan an independent reconciliation that does not reuse the same logic as the result being checked.
...
1. Run the broken query. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. State input and output grain. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Measure row multiplication. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Identify the cardinality error. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Rewrite and reconcile the result. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: DISTINCT is not used as an unexplained patch.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Revenue reconciles.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: The exact cardinality error is named.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Corrected SQL.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Before-and-after reconciliation.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Root-cause explanation.. Make its purpose, scope, and source clear to a reviewer.
