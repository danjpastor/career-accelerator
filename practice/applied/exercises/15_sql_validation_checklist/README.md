...
Use SQL to run three focused checks on a small dataset: row count, missing values, and duplicate or relationship risk.
...
1. Read the assignment and restate the goal in one sentence: Use SQL to run three focused checks on a small dataset: row count, missing values, and duplicate or relationship risk.
...
1. Write a row-count check and state what one row represents.
2. Write one missing-value check and one duplicate or relationship check.
3. Summarize which check passed, which issue needs attention, and why it matters.

### Required output

One SQL file containing the three checks. A short quality summary.

### Check your work

- At least three focused checks run successfully.
- Each check names the field or relationship being tested.
- The summary matches the query results.
...
1. Verify: At least three focused checks run successfully.
2. Verify: Each check names the field or relationship being tested.
3. Verify: The summary matches the query results.
