...
correlation, confounding, reverse causality, omitted variables, causal claim, observational evidence

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to review a strong observed relationship, identify plausible confounders, and rewrite an unsupported causal claim..
...
3. List the concepts you expect to use: correlation, confounding, reverse causality, omitted variables, causal claim, observational evidence.
...
1. Measure and describe the observed association. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. List plausible confounders and reverse-causality explanations. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Identify what the current data can and cannot establish. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Propose an experiment or quasi-experimental design. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Rewrite the conclusion using appropriately cautious language. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Correlation is not treated as proof of causation.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: At least one plausible alternative mechanism is considered.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: The proposed design addresses the main confounder.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Association summary.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Confounder and alternative-explanation list.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Revised claim and stronger study proposal.. Make its purpose, scope, and source clear to a reviewer.
