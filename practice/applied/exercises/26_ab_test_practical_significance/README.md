...
experiment design, random assignment, conversion lift, confidence interval, practical significance, guardrail metrics

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to evaluate whether an experiment is statistically credible, commercially meaningful, and safe to roll out..
...
3. List the concepts you expect to use: experiment design, random assignment, conversion lift, confidence interval, practical significance, guardrail metrics.
...
1. Check group balance and assignment integrity. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Calculate conversion, absolute lift, and relative lift. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Estimate uncertainty around the treatment effect. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Compare the effect with a predefined minimum practical threshold. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Review revenue and response-time guardrails before recommending rollout. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Group sizes and baseline characteristics are checked.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: The recommendation includes uncertainty and guardrails.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: A statistically significant but immaterial effect is treated appropriately.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Experiment scorecard.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Statistical and practical significance assessment.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Rollout, hold, or rerun recommendation.. Make its purpose, scope, and source clear to a reviewer.
