...
hallucinated fields, duplicated joins, denominator errors, unsupported claims, chart risk, verification trail

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to review a plausible AI-generated query, summary, and chart recommendation; identify every unsupported or incorrect element; and produce a verified replacement..
...
3. List the concepts you expect to use: hallucinated fields, duplicated joins, denominator errors, unsupported claims, chart risk, verification trail.
...
1. Separate AI-generated claims from facts that can be verified directly in the data or source material.
2. List every calculation, citation, transformation, and recommendation that needs independent checking.
3. Identify privacy, bias, hallucination, and overconfidence risks before editing the output.
4. Define what evidence would be required before a stakeholder could safely act on the analysis.
...
1. Identify nonexistent or misused fields. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Diagnose join duplication and denominator errors. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Check every numerical claim against source data. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Separate observation, inference, and recommendation. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Replace the misleading chart recommendation. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
6. Document a reusable AI-output verification checklist. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Every correction links to a validation check.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Unsupported causal or universal claims are removed.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: The final analysis can be reproduced without trusting the AI output.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Issue log.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Corrected query and verified results.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Rewritten summary and chart recommendation.. Make its purpose, scope, and source clear to a reviewer.
