...
metric risk, stakeholder clarification, safer alternative, ethical communication

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to handle a metric request that could encourage the wrong behavior or misrepresent performance..
...
3. List the concepts you expect to use: metric risk, stakeholder clarification, safer alternative, ethical communication.
...
1. Read the request once for the business question and again for the deadline, scope, and required output.
2. Timebox source inspection, analysis, validation, and communication before starting.
3. Define the minimum defensible answer and avoid optional analysis until the core result is validated.
4. Keep a short decision log so assumptions made under time pressure remain visible.
...
1. Restate the requested metric and decision. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Identify gaming or interpretation risk. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Ask one clarifying question. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Propose a safer metric or balanced set. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Draft a respectful response. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Alternative is measurable.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Response preserves the business goal.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: New metric reduces the identified risk.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Risk assessment.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Revised metric definition.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Stakeholder response.. Make its purpose, scope, and source clear to a reviewer.
