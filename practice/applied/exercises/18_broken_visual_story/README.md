...
Identify how a chart and narrative exaggerate a change and rebuild them with context.

## Skills you will apply

chart scale, cherry-picking, average of averages, annotation, uncertainty

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to identify how a chart and narrative exaggerate a change and rebuild them with context..
...
3. List the concepts you expect to use: chart scale, cherry-picking, average of averages, annotation, uncertainty.
...
1. List misleading visual and statistical choices. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Recalculate selectively summarized metrics. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Choose an appropriate scale and period. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Rewrite the headline. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Add a meaningful limitation or annotation. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Material context is preserved.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Headline is supported.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Limitations are visible.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Corrected chart or redesign.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Corrected headline and paragraph.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: List of misleading choices.. Make its purpose, scope, and source clear to a reviewer.
