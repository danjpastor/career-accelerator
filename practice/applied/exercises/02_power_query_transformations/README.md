# Applied Lab 15: Clean and combine two tables with Power Query
...
Use Power Query to clean two supplied tables and combine them with one merge or append operation.
...
1. Read the assignment and restate the goal in one sentence: Use Power Query to clean two supplied tables and combine them with one merge or append operation.
...
1. Apply only the required text, type, and missing-value fixes to each table.
2. Combine the tables using the relationship or stacking method described in the prompt.
3. Check the output row count and one key total against the inputs.

### Required output

One cleaned combined query in Power BI. A short reconciliation note.
...
1. Verify: Transformation steps have readable names.
2. Verify: The combination method matches the table grain.
3. Verify: The output row count or total reconciles to the inputs.
