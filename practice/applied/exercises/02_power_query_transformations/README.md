...
Power Query cleaning, merge, append, pivot, unpivot

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to create a repeatable transformation flow that converts raw exports into analysis-ready tables..
...
3. List the concepts you expect to use: Power Query cleaning, merge, append, pivot, unpivot.
...
1. Trim and standardize text and replace invalid blanks with nulls. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Merge shots to projects and verify join cardinality. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Append the monthly time-entry files. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Unpivot the wide stage-target table. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: The merge does not duplicate shot grain.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: The appended total equals both inputs combined.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: The unpivoted data reconciles to the wide source.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Transformation-flow screenshots or notes.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Before-and-after row counts.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Explanation of why the long target table is easier to analyze.. Make its purpose, scope, and source clear to a reviewer.
