...
Turn the assignment into a clear analytical question before using a tool. The lab objective is to build a reproducible ingestion script using local API-response fixtures, including pagination, errors, duplicate records, and missing optional fields..
...
3. List the concepts you expect to use: HTTP status, pagination, JSON parsing, schema drift, retries, missing fields, deduplication, extraction timestamp.
...
1. Document the source, endpoint or file location, expected pagination behavior, and any access limitations.
2. Inspect one response or file before designing the full ingestion process.
3. Identify stable record keys and decide how nested fields will be represented.
4. Plan retry, duplicate, missing-page, and schema-change checks before collecting the full dataset.
...
1. Read the first response and follow next-page references. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Handle a simulated rate-limit or server-error response. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Flatten nested customer and order fields. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Detect duplicate identifiers and schema changes. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Add extraction timestamp, source page, and row-count logging. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Every page is processed exactly once.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Duplicate IDs are detected explicitly.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Missing optional fields do not crash the ingestion.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Runnable ingestion script.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Flattened CSV output.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Extraction and validation log.. Make its purpose, scope, and source clear to a reviewer.
