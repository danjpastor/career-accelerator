...
Estimate uncertainty around a sample mean and proportion and communicate what a confidence interval does and does not mean.

## Skills you will apply

standard error, confidence level, confidence interval, margin of error, sample size

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to estimate uncertainty around a sample mean and proportion and communicate what a confidence interval does and does not mean..
...
3. List the concepts you expect to use: standard error, confidence level, confidence interval, margin of error, sample size.
...
1. Calculate a sample mean, standard deviation, and standard error. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Build a 95% confidence interval for mean response time. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Build a confidence interval for a conversion proportion. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Compare interval width for two sample sizes. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Write a stakeholder-friendly interpretation without claiming a probability about the fixed parameter. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Standard error uses sample size correctly.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: The interpretation avoids saying 95% of individual values are inside the interval.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: The effect of sample size on precision is explained.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Runnable calculations.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Confidence-interval comparison table.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Plain-language interpretation.. Make its purpose, scope, and source clear to a reviewer.
