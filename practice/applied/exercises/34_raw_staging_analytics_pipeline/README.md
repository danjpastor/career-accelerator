...
Turn the assignment into a clear analytical question before using a tool. The lab objective is to transform inconsistent raw extracts into documented staging, clean, and analytical layers that can be rebuilt safely..
...
3. List the concepts you expect to use: raw layer, staging views, clean layer, analytical marts, idempotency, validation, lineage.
...
1. Draw the intended path from raw inputs to cleaned, validated, and analysis-ready outputs.
2. Define what is allowed to change at each layer and what must remain immutable.
3. Choose stable keys, naming rules, and validation checks before building transformations.
4. Plan how another analyst could rerun the workflow without relying on undocumented manual steps.
...
1. Load immutable raw monthly files with source-file metadata. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Create staging views that standardize names and data types. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Create clean tables with deduplication and explicit rejection logic. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Create an analytical mart at a documented grain. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Add row-count, uniqueness, referential, and reconciliation checks. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
6. Create a lineage document from source to final metric. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Rerunning the workflow does not duplicate results.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Rejected records are counted and retained for review.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Final totals reconcile to accepted raw records.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Layered SQL workflow.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Validation report.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Data-lineage diagram or table.. Make its purpose, scope, and source clear to a reviewer.
