...
CALCULATE, DIVIDE, conditional logic, variance, time comparison

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to create a reusable measure layer and independently validate its results..
...
3. List the concepts you expect to use: CALCULATE, DIVIDE, conditional logic, variance, time comparison.
...
1. Create total hours, shot count, completion rate, and average-hours measures. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Use DIVIDE and CALCULATE appropriately. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Create budget and schedule variance measures. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Add a prior-period comparison. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Document formulas and business meaning. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Measures respond correctly to filters.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Rates use safe division.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Totals reconcile to another tool.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Measure dictionary.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Validation table comparing measures to source totals.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Note on one filter-context issue.. Make its purpose, scope, and source clear to a reviewer.
