...
KPI denominator, inclusive dates, weighted rates, analysis grain

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to repair a wrong denominator, incomplete date filter, and average-of-averages error..
...
3. List the concepts you expect to use: KPI denominator, inclusive dates, weighted rates, analysis grain.
...
1. Define the KPI in words. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Identify numerator and denominator populations. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Correct the date boundary. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Replace average-of-averages with a weighted result. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Compare business interpretation before and after. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Denominator matches stated population.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Date boundaries are tested.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Weighted result reconciles.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Corrected SQL and definitions.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Broken-vs-corrected table.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Largest decision impact.. Make its purpose, scope, and source clear to a reviewer.
