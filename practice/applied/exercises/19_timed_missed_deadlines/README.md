# Applied Lab 11: Answer a missed-deadlines question
...
Answer one narrowly scoped missed-deadlines question within a fixed 30-minute workflow.
...
1. Read the assignment and restate the goal in one sentence: Answer one narrowly scoped missed-deadlines question within a fixed 30-minute workflow.
...
1. Define the requested population, deadline rule, and output before querying.
2. Produce one validated result table that answers the request.
3. Write a three-bullet response covering result, action, and limitation.

### Required output

One result table. A three-bullet stakeholder response.
...
1. Verify: The deadline rule is explicit.
2. Verify: The result table answers only the requested scope.
3. Verify: One quick reconciliation or sample check is recorded.
