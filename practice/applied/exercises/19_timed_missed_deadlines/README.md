...
Answer a manager who needs the top causes of missed deadlines within 30 minutes.

## Skills you will apply

rapid scoping, SQL analysis, validation, concise recommendation

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to answer a manager who needs the top causes of missed deadlines within 30 minutes..
...
3. List the concepts you expect to use: rapid scoping, SQL analysis, validation, concise recommendation.
...
1. Write one clarifying question. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Define missed deadline and grain. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Find the top three causes and contribution. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Run two checks. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Write a three-sentence conclusion and action. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Count and rate are distinguished.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Time window is stated.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Recommendation is proportional to evidence.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Method or SQL.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Validation results.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Manager-ready response.. Make its purpose, scope, and source clear to a reviewer.
