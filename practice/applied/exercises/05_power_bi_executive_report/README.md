...
visual hierarchy, slicers, drill-through, tooltips, accessibility

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to turn the model into a concise report that supports a named management decision..
...
3. List the concepts you expect to use: visual hierarchy, slicers, drill-through, tooltips, accessibility.
...
1. Define the audience, decision, and three questions. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Build an overview page with clear visual hierarchy. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Add useful slicers, drill-through, and a tooltip page. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Check titles, units, contrast, alt text, and layout. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Remove visuals that do not support a question. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Every visual answers a named question.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: The report fits common laptop resolution.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Titles remain meaningful after filters.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Overview and detail pages.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Portfolio-ready screenshots.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Decision supported by each page.. Make its purpose, scope, and source clear to a reviewer.
