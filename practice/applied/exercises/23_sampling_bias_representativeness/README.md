...
population, sample, sampling frame, selection bias, nonresponse bias, representativeness

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to compare a full population with a convenience sample and explain how coverage and nonresponse bias change the conclusion..
...
3. List the concepts you expect to use: population, sample, sampling frame, selection bias, nonresponse bias, representativeness.
...
2. Compare demographic and behavioral proportions between population and sample. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Identify groups that are overrepresented or missing. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Explain how the bias could affect the reported outcome. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Recommend a safer collection or weighting strategy. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Representativeness is evaluated across multiple relevant fields.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: The analysis distinguishes sampling bias from random sampling error.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: The recommendation is practical for the stated collection process.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Population-versus-sample comparison table.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Bias assessment.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Recommended sampling improvement.. Make its purpose, scope, and source clear to a reviewer.
