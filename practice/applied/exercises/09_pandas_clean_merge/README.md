...
string cleaning, dates, missing values, merge validation, duplicate control

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to create a clean analysis table while protecting its intended grain..
...
3. List the concepts you expect to use: string cleaning, dates, missing values, merge validation, duplicate control.
...
1. Standardize text and dates. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Handle missing identifiers explicitly. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Merge orders, customers, products, and returns with validation. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Create a row-count reconciliation table. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Export clean detail data. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: One row per intended order line.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Unexpected many-to-many merges fail.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Excluded rows are counted and explained.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Cleaning script.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Clean output CSV.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Reconciliation report.. Make its purpose, scope, and source clear to a reviewer.
