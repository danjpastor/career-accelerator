...
Clean the join fields in two small pandas tables, merge them safely, and produce one grouped summary.
...
1. Read the assignment and restate the goal in one sentence: Clean the join fields in two small pandas tables, merge them safely, and produce one grouped summary.
...
1. Load both tables and clean only the key text, date, and missing-value fields needed for the merge.
2. Check key uniqueness, perform the merge with validation, and inspect unmatched rows.
3. Create one grouped summary from the merged data and write a two-sentence takeaway.

### Required output

One runnable Python script or notebook. One merged summary table and a two-sentence takeaway.

### Check your work

- The merge validation matches the expected relationship.
- Unmatched and duplicate keys are checked explicitly.
- The grouped summary reconciles to the merged rows.
...
1. Verify: The merge validation matches the expected relationship.
2. Verify: Unmatched and duplicate keys are checked explicitly.
3. Verify: The grouped summary reconciles to the merged rows.
