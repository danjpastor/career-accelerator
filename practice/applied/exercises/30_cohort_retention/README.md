...
Turn the assignment into a clear analytical question before using a tool. The lab objective is to build a cohort-retention table that distinguishes customer age from calendar month and handles incomplete recent cohorts..
...
3. List the concepts you expect to use: cohort month, activity month, period index, retained users, retention rate, incomplete cohorts.
...
1. Assign each user to a first-activity cohort. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Calculate activity period relative to the cohort month. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Count retained users and divide by the original cohort size. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Build a long and pivoted retention table. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Each user belongs to one cohort.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Retention never exceeds 100%.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Incomplete recent cohorts are not mistaken for zero retention.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Cohort SQL.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Retention matrix.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Interpretation of the strongest and weakest cohorts.. Make its purpose, scope, and source clear to a reviewer.
