...
mean, median, variance, standard deviation, percentiles, distribution shape, outliers

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to summarize a dataset with appropriate descriptive statistics, recognize skewed distributions, and decide when an outlier is an error, an exception, or an important business signal..
...
3. List the concepts you expect to use: mean, median, variance, standard deviation, percentiles, distribution shape, outliers.
...
1. Profile numeric fields using count, mean, median, standard deviation, and percentiles. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Compare mean and median for skewed variables. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Create distribution summaries for response time, revenue, and satisfaction. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Identify potential outliers using IQR and a business-rule threshold. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Explain which summary statistic is safest for each business question. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Missing values are excluded or handled explicitly.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: At least one skewed metric is interpreted with the median.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Outliers are investigated rather than deleted automatically.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: A runnable analysis script.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: A descriptive-statistics table.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: A short interpretation of skew, spread, and outliers.. Make its purpose, scope, and source clear to a reviewer.
