...
Fit a basic regression, validate its assumptions at an introductory level, and explain the coefficient without making a causal claim.

## Skills you will apply

dependent variable, predictor, slope, intercept, residual, R-squared, extrapolation, coefficient interpretation

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to fit a basic regression, validate its assumptions at an introductory level, and explain the coefficient without making a causal claim..
...
3. List the concepts you expect to use: dependent variable, predictor, slope, intercept, residual, R-squared, extrapolation, coefficient interpretation.
...
1. Define the outcome and predictor in business terms. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Fit a simple linear model. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Interpret slope, intercept, and R-squared. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Inspect residual patterns and influential observations. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Explain why prediction outside the observed range is risky. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: Units are included in the coefficient interpretation.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Association is not described as causation.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Residuals and extrapolation risk are discussed.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Runnable model.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Coefficient and fit summary.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Plain-language interpretation and limitations.. Make its purpose, scope, and source clear to a reviewer.
