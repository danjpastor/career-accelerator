...
event grain, funnel stages, stage conversion, overall conversion, drop-off, segmentation

## Stage 1: Frame the request and define success

Turn the assignment into a clear analytical question before using a tool. The lab objective is to construct a correctly ordered funnel, quantify drop-off, and identify where a business should investigate first..
...
3. List the concepts you expect to use: event grain, funnel stages, stage conversion, overall conversion, drop-off, segmentation.
...
1. Rewrite the requested metric in plain language and identify the business decision it should support.
2. Define the entity, time period, numerator, denominator, exclusions, and segmentation rules.
3. Sketch the final table before building it so each row and column has a clear meaning.
4. Choose at least one independent check that would reveal duplicated entities or inconsistent populations.
...
1. Define the user and event grain. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
2. Create mutually consistent funnel-stage eligibility rules. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
3. Calculate users reaching each stage and stage-to-stage conversion. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
4. Segment results by acquisition channel. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
5. Recommend one next diagnostic based on the largest material drop-off. Before you begin, identify which source fields and assumptions this step depends on. Complete the work in a clearly labeled section of the submission, then run a small check that would expose a missing row, duplicate entity, incorrect filter, or mismatched unit.
...
1. Verify this requirement independently: A user cannot reach an earlier count through duplicate events.. Record how you checked it rather than only stating that it passed.
2. Verify this requirement independently: Stages occur in the required order.. Record how you checked it rather than only stating that it passed.
3. Verify this requirement independently: Stage conversion and overall conversion are labeled separately.. Record how you checked it rather than only stating that it passed.
...
1. Create the required deliverable: Funnel SQL.. Make its purpose, scope, and source clear to a reviewer.
2. Create the required deliverable: Overall and segmented funnel table.. Make its purpose, scope, and source clear to a reviewer.
3. Create the required deliverable: Business interpretation.. Make its purpose, scope, and source clear to a reviewer.
